...
public class StudyViewFilter implements Serializable {
	...
	private static final long serialVersionUID = -3529813193969077157L;
	...
	private <T extends DataFilter> boolean validateDataFilters(List<T> dataFilters) {
		...
		if (dataFilters != null) {
			invalidCount = dataFilters.stream().flatMap(f -> f.getValues().stream()).filter(Objects::nonNull)
					.filter(v -> v.getValue() != null == (v.getStart() != null || v.getEnd() != null)).count();
		}
	...
	public void setPatientTreatmentTargetFilters(AndedPatientTreatmentFilters patientTreatmentTargetFilters) {
		this.patientTreatmentTargetFilters = patientTreatmentTargetFilters;
	}
